...
setwd("/Users/lauren/Downloads/core-metrics-results-grouped") #you can paste your directory to move there
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=groups)) +
  geom_point() + #alpha controls transparency and helps when points are overlapping
  theme_q2r() +
  xlab("PC1 (32.27%)") +
  ylab("PC2 (22.28%)") +
  scale_color_manual(values=c("Black", "Green"), name = "group")

# Now we are going to make our code a little more re-usable
groups <- c("Black", "Blue", "Green", "Gray")
my_column <- "body.site"
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=groups, name = my_column)
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=groups, name = "Body Site")
